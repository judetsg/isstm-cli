...
# this requires that the audio_grabber.py has already been lauched
# because it start the push-pull server and the audio capture
# that enables speech to text at the same time
context = zmq.Context()
...
def saisir_note_examen():
    while True:
        ...
        if annee_choisi == "Retour":
            break
        redis_client.set('annee', annee_choisi)
        # Come back here if activite == retour
        while True:
            # 1. Choisir le niveau
            niveaux_choices = utility_functions.select_niveau(conn)
            niveau_choisi = chooser.selector(
                "Choisissez un niveau", niveaux_choices)
            # sauvegarder sur redis
            if niveau_choisi == "Retour":
                break
            ...
            while True:
                ...
                # Revenir ici si le choix est 999
                while True:

                    # 4. choisir si on veut saisir les codes ou les notes
                    activite = chooser.selector('Type de Saisie', [
                        questionary.Choice(
                            title='Anonymat', value='saisie_code'),
...
def generate_json():
    ...
    etudiant_id = chooser.tuple_selector('Choisir un étudiant: ',
                                         utility_functions.select_etudiant(conn, niveau_choisi, annee=annee_choisi), socket)
